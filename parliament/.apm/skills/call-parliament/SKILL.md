...
1. 成果物がチェックリストを満たしているか審査。**`[critical]` タグが存在する場合は `is_critical: true` の全項目が `PASS` であることを `APPROVED` の条件とする**。`is_critical: false` の FAIL は `residual_risks` として受理する。
2. `[critical]` 項目が全 PASS（または `[critical]` タグなしで全項目 PASS）→ `APPROVED`
3. いずれかの `[critical]` 項目が FAIL → `REJECTED` (差し戻し理由を critical 項目のみ明記して再生成)
