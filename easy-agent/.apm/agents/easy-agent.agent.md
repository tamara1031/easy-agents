...
| call-refine-loop | (スキル直接呼び出し — `agent` ツール不要) |

### call-refine-loop (refine-loop スキル呼び出し)

Verify フェーズで成果物を外部レビューし、反復改善する。
...
* **Verify**: `refine-loop` スキルを呼び出し、外部レビュアーが requirements_checklist を2連続 CONVERGED と判定したか。自己評価ループ（REVISE）は Verify フェーズでは使用しない。
