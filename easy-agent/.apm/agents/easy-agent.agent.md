...
agent(
  ...
  prompt: """
    ...
    requirements_checklist:
      - "[critical] <必須要件>"
      - "<通常要件>"
      - "<residual_risks[N]>  # Hierarchy 委譲後: [critical] タグなしで追記 (ADR-019)"
...
> **Hierarchy 委譲後の `residual_risks` 引き継ぎ (ADR-019)**: call-hierarchy が返した `manager_output.residual_risks`（non-critical FAIL の記録）は `task_context` ではなく `requirements_checklist` へ `[critical]` タグなし項目として追記すること。これにより refine-loop の reviewer が各リスクを明示的に PASS/FAIL 判定できる。
