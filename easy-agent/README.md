...
| APM 宣言依存 | `advisor`, `parliament`, `taskforce`, `memoir`（`apm.yml` の `dependencies.apm`） |
...
`apm.yml` で `advisor` / `parliament` / `taskforce` / `memoir` を宣言依存しているため、`apm install` 時に自動的に解決・インストールされます。
