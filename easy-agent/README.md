...
| 項目 | 要件 |
| :--- | :--- |
| APM バージョン | `apm.yml` に基づく（依存パッケージなし） |
| パッケージバージョン | 0.1.0 |
...
`apm.yml` に外部依存パッケージはないため、追加インストールは不要です。
...
├── apm.yml                   # APM プロジェクト設定。パッケージ名・バージョン・依存関係を管理
├── apm.lock.yaml             # APM 依存関係のロックファイル
...
**`apm.yml`**
APM プロジェクトの設定ファイルです。現在は外部依存パッケージなし（`dependencies.apm: []`）で動作します。
