...
| [preflight-sync-runbook.md](./preflight-sync-runbook.md) | main 同期の運用ランブック |
...
定期バッチや自動実行ジョブで `main` 同期を行う場合は、[Pre-flight Sync Runbook](./preflight-sync-runbook.md) を基準手順として利用する。
