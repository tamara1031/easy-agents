...
- `easy-agent.agent.md` の Auto-Memory Protocol を `.apm/hooks/` または同等の hook 仕様ファイルへ物理的に抽出する（独立 ADR で扱う） — **形式は [ADR-011](./ADR-011-hook-specification-format.md) で確定済み。残るは物理ディレクトリ規約の決定。**
...
- ~~Hook の発火イベント語彙の標準化（SessionStart / PhaseGateComplete / NExchangeElapsed 等）~~ → [ADR-011](./ADR-011-hook-specification-format.md) で解決
