...
1. 競合ファイルを確認: `git status --short`
2. 競合マーカーを探索: `rg -n "^(<<<<<<<|=======|>>>>>>>)" <file>`
3. マーカーを除去して論理的に統合。
4. `git add <file>` 後に `git commit`。

> 注意: 競合マーカーを残したままのコミットは禁止。

### 競合解消の完了条件 (Definition of Done) **[critical]**

以下をすべて満たしたら競合解消完了とみなす。

- 競合ファイルが 0 件: `git status --short` に `UU` / `AA` / `DD` がない
- 競合マーカーが 0 件: `rg -n "^(<<<<<<<|=======|>>>>>>>)" .`
- 解消コミット後にテスト/リンターが再実行され、失敗がない
